...
class SoundManager {
    ...
    toggle() {
        ...
        if (btnDesktop) btnDesktop.textContent = this.enabled ? '🔊' : '🔇';
    }
...
const SYMBOLS = {
    ...
    8: { id: 8, name: 'Scatter', value: 100, type: 'scatter', svg: '<svg viewBox="0 0 100 100"><defs><linearGradient id="bagBrown" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#D2691E"/><stop offset="50%" stop-color="#A0522D"/><stop offset="100%" stop-color="#8B4513"/></linearGradient><linearGradient id="rope" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stop-color="#F4A460"/><stop offset="100%" stop-color="#CD853F"/></linearGradient></defs><ellipse cx="50" cy="75" rx="32" ry="20" fill="url(#bagBrown)" stroke="#654321" stroke-width="2"/><path d="M 20 55 Q 20 25 50 20 Q 80 25 80 55 L 75 75 Q 75 80 50 85 Q 25 80 25 75 Z" fill="url(#bagBrown)" stroke="#654321" stroke-width="2.5"/><path d="M 35 22 Q 35 18 50 17 Q 65 18 65 22" fill="none" stroke="#654321" stroke-width="2"/><ellipse cx="50" cy="20" rx="15" ry="4" fill="url(#rope)" stroke="#8B4513" stroke-width="1.5"/><path d="M 35 20 L 32 28 Q 30 32 35 33" fill="none" stroke="url(#rope)" stroke-width="3" stroke-linecap="round"/><path d="M 65 20 L 68 28 Q 70 32 65 33" fill="none" stroke="url(#rope)" stroke-width="3" stroke-linecap="round"/><text x="50" y="55" font-family="Impact" font-size="28" font-weight="900" text-anchor="middle" fill="#FFD700" stroke="#654321" stroke-width="1">Bs</text><circle cx="35" cy="65" r="3" fill="#FFD700" opacity="0.7"/><circle cx="65" cy="68" r="2.5" fill="#FFD700" opacity="0.7"/><circle cx="50" cy="72" r="2" fill="#FFD700" opacity="0.6"/></svg>' }
};

// Solo para visualización inicial, el servidor decide el resultado
...
class SlotMachine {
    ...
    init() {
        ...
        [5, 10, 25, 50].forEach(amount => {
            const btn = document.getElementById(`btn-bet-${amount}`);
            if (btn) btn.addEventListener('click', () => this.adjustBetTotal(amount));
        });

        // Audio Toggle
        const btnSound = document.getElementById('btn-sound');
        if (btnSound) btnSound.addEventListener('click', () => audio.toggle());
        
        const btnSoundDesktop = document.getElementById('btn-sound-desktop');
        if (btnSoundDesktop) btnSoundDesktop.addEventListener('click', () => audio.toggle());
    }

    setupSocket() {
        const joinGame = () => {
            console.log('Conectado a Tragamonedas - Enviando joinTragamonedas');
            socket.emit('joinTragamonedas');
        };
        ...
        socket.on('tragamonedasInit', (data) => {
            this.balance = data.balance;
            this.username = data.username;
            this.updateUI();
        });

        socket.on('tragamonedasResult', (data) => {
            if (data.error) {
                alert(data.error);
                this.isSpinning = false;
                this.updateUI();
            ...
            this.startSpinAnimation(data.grid, data.totalWin, data.newBalance, data.winLines, data.scatterCount);
        });
    }
    ...
    spin() {
        ...
        // 1. Pedir al servidor que gire
        this.isSpinning = true;
        // Descontamos visualmente para feedback inmediato (el servidor valida igual)
        this.balance -= this.totalBet; 
        this.updateUI();
        this.clearOverlays();

        socket.emit('spinTragamonedas', { totalBet: this.totalBet });
    }

    startSpinAnimation(serverGrid, totalWin, newBalance, winLines, scatterCount) {
        this.grid = serverGrid; // Guardamos el resultado del servidor
        ...
        this.reels.forEach((reel, index) => {
            ...
            const loop = () => {
                if (!reel.stopping) {
                    ...
                    // Preparamos la tira final con el resultado del servidor
                    // El servidor manda grid[col][row]. 
                    // grid[index][0] es arriba, [1] medio, [2] abajo.
                    const finalSymbols = [
                        this.grid[index][0],
                        this.grid[index][1],
                        this.grid[index][2],
                    ...
                    reel.element.style.transform = 'translateY(0%)';
                    audio.reelStop();
                    ...
                    setTimeout(() => {
                        ...
                        if (completedReels === 3) {
                            this.isSpinning = false;
                            this.showResults(totalWin, newBalance, winLines, scatterCount);
                        }
                    }, 400);
                }
            };
    ...
    showResults(totalWin, newBalance, winLines, scatterCount) {
        // Actualizamos saldo real confirmado por servidor
        this.balance = newBalance;
        this.updateUI();

        if (scatterCount >= 3) {
             this.highlightSymbolType(8); // Scatter ID
        }

        winLines.forEach((lineIdx) => {
            const line = this.paylines[lineIdx];
            // Color cíclico
            const color = ['#ff0033', '#00f3ff', '#ffd700'][lineIdx % 3];
            this.drawPayline(line, color);
            
            line.forEach(p => {
                // p[0] es columna, p[1] es fila (0,1,2)
                // En el DOM, los hijos 0,1,2 son los visibles
                const dom = this.reels[p[0]].element.children[p[1]];
    ...
    drawPayline(coords, color) {
        ...
        if (!isMobileOrTablet) {
            ...
            paddingTop = parseFloat(gameAreaStyle.paddingTop);
            paddingLeft = parseFloat(gameAreaStyle.paddingLeft);
            borderTop = parseFloat(gameAreaStyle.borderTopWidth);
            borderLeft = parseFloat(gameAreaStyle.borderLeftWidth);
        }
    ...
    highlightSymbolType(id) {
        this.reels.forEach((reel, c) => {
            this.grid[c].forEach((symId, r) => {
                if (symId === id) reel.element.children[r].classList.add('win-anim');
            });
        });
    }
